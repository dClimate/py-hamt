...
bash run-checks.sh
```
This will run tests with code coverage, and check formatting and linting. If a local ipfs daemon is not running it will not run all tests, but it will spawn a docker ipfs container if docker is installed and run as many integration tests as possible.
