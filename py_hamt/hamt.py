...
class HAMT(MutableMapping):
    """
    This HAMT presents a key value interface like a python dictionary, except keys can only be strings and values can only be types amenable with [IPLDKind](https://dag-cbor.readthedocs.io/en/stable/api/dag_cbor.ipld.html#dag_cbor.ipld.IPLDKind). IPLDKind is a fairly flexible data model but note that integers are limited to the limits of a signed 64-bit integer.
    ...
    # IPLDKind type for getting and setting

    Although not generated by the documentation, note that `HAMT` can store any type that aligns with IPLDKind.
    """
