...
async def handle_pin(args):
    ...
    """
    async with KuboCAS(rpc_base_url=args.rpc_url, gateway_base_url=args.gateway_url) as kubo_cas:
        try:
            print(f"-> Opening store with root CID: {args.root_cid}")
            store = await ShardedZarrStore.open(cas=kubo_cas, read_only=True, root_cid=args.root_cid)
        except Exception as e:
            print(f"Error: Failed to open Zarr store for CID {args.root_cid}. Ensure the CID is correct and the daemon is running.", file=sys.stderr)
...
def main():
    ...
    parser.add_argument('--rpc-url', default='http://127.0.0.1:5001', help='IPFS Kubo RPC API endpoint URL.')
    parser.add_argument('--gateway-url', default='http://127.0.0.1:8080', help='IPFS Gateway URL (needed for loading shards).')

    subparsers = parser.add_subparsers(dest='command', required=True, help='Available commands')

    # --- Pin Command ---
    parser_pin = subparsers.add_parser('pin', help='Recursively pin a dataset using its root CID.')
    parser_pin.add_argument('root_cid', help='The root CID of the dataset to pin.')
    parser_pin.set_defaults(func=handle_pin)

    # --- Unpin Command ---
    parser_unpin = subparsers.add_parser('unpin', help='Recursively unpin a dataset using its root CID.')
